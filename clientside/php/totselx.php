<?php
    ...
    $timofy=date("Y");
    // for day
    $query = $con->prepare("SELECT SUM(`purexcoast`) FROM `companyselex` WHERE `domaine` = ? AND `totday` = ?" );
    ...
    // for month
    $querymon = $con->prepare("SELECT SUM(`purexcoast`) FROM `companyselex` WHERE `domaine` = ? AND `totmont` = ?" );
    ...
    //for year
    $queryyear = $con->prepare("SELECT SUM(`purexcoast`) FROM `companyselex` WHERE `domaine` = ? AND `totyear` = ?" );
